...
def transfer_to_device(x, device):
    """
    Transfers PyTorch tensors or lists of tensors to GPU.
...
def parse_configuration(config_file):
    """
    Load config file if a string was passed and returns the input if a dictionary was passed
    """
...
def stack_all(list, dim = 0):
    """
    Stack all iterables of torch tensors in a list
    i.e. [[(tensor), (tensor)], [(tensor, tensor)]]
    """
    return [torch.stacks(s,dim) for s in list]
